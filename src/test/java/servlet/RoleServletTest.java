...
import org.example.servlet.RoleServlet;
...
class RoleServletTest {
    ...
    void doPost() throws IOException {
        ...
        ArgumentCaptor<RoleIncoming> argumentCaptor = ArgumentCaptor.forClass(RoleIncoming.class);
        Mockito.verify(mockRoleService).save(argumentCaptor.capture());

        RoleIncoming result = argumentCaptor.getValue();
    ...
    void doPut() throws IOException, NotFoundException {
        ...
        ArgumentCaptor<RoleUpdate> argumentCaptor = ArgumentCaptor.forClass(RoleUpdate.class);
        Mockito.verify(mockRoleService).update(argumentCaptor.capture());

        RoleUpdate result = argumentCaptor.getValue();
        Assertions.assertEquals(expectedName, result.getName());
    }
    ...
    void doPutNotFound() throws IOException, NotFoundException {
        ...
        Mockito.doThrow(new NotFoundException("not found.")).when(mockRoleService)
                .update(Mockito.any(RoleUpdate.class));

        roleServlet.doPut(mockRequest, mockResponse);

        Mockito.verify(mockResponse).setStatus(HttpServletResponse.SC_NOT_FOUND);
        Mockito.verify(mockRoleService).update(Mockito.any(RoleUpdate.class));
    }
