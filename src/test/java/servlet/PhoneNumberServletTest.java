...
import org.example.servlet.PhoneNumberServlet;
...
class PhoneNumberServletTest {
    ...
    void doPost() throws IOException {
        ...
        ArgumentCaptor<PhoneNumberIncoming> argumentCaptor = ArgumentCaptor.forClass(PhoneNumberIncoming.class);
        Mockito.verify(mockPhoneNumberService).save(argumentCaptor.capture());

        PhoneNumberIncoming result = argumentCaptor.getValue();
    ...
    void doPut() throws NotFoundException, IOException {
        ...
        ArgumentCaptor<PhoneNumberUpdate> argumentCaptor = ArgumentCaptor.forClass(PhoneNumberUpdate.class);
        Mockito.verify(mockPhoneNumberService).update(argumentCaptor.capture());

        PhoneNumberUpdate result = argumentCaptor.getValue();
        Assertions.assertEquals(expectedNumber, result.getNumber());
